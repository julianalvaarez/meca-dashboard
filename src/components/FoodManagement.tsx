...
import {
    getAllFoodStats, checkFoodStatsExistence, addFoodStats, deleteFoodStats, updateFoodStats
} from "@/lib/food.service"
import { Loader2, Plus, Pencil, Trash2, UtensilsCrossed, AlertCircle, CheckCircle2 } from "lucide-react"
...
const formSchema = z.object({
    ...
    total_expense: z.coerce.number().min(0, "Mínimo 0"),
})
...
export function FoodManagement({ onRefresh }: { onRefresh: () => void }) {
    ...
    useEffect(() => {
        async function check() {
            if (watchYear && watchMonth && !editingRecord) {
                ...
                const exists = await checkFoodStatsExistence(parseInt(watchYear), parseInt(watchMonth))
    ...
    const fetchRecords = async () => {
        ...
        const res = await getAllFoodStats()
        if (res.success) {
            setRecords(res.data || [])
        } else {
    ...
    const onSubmit = async (values: FormValues) => {
        ...
        const res = editingRecord
            ? await updateFoodStats(editingRecord.id, dataToSave)
            : await addFoodStats(dataToSave)
    ...
    const handleDelete = async (id: number) => {
        setSubmitting(true)
        const res = await deleteFoodStats(id)
        if (res.success) {
            ...
            <CardHeader className="flex flex-row items-center justify-between pb-6 border-b">
                ...
                <Dialog open={isAddOpen} onOpenChange={(open) => {
                    ...
                    <DialogContent className="max-w-md">
                        <DialogHeader>
                            <DialogTitle>{editingRecord ? "Editar Registro" : "Agregar Estadísticas Gastronómicas"}</DialogTitle>
                            <DialogDescription>
                                Ingrese la información de ingresos y costos por materia prima.
                            </DialogDescription>
                        </DialogHeader>
                            ...
                            <form onSubmit={form.handleSubmit(onSubmit) as any} className="space-y-6">
                                ...
                                <div className="space-y-4 pt-2">
                                    <FormField control={form.control} name="total_income" render={({ field }) => (
                                        <FormItem>
                                            <FormLabel>Ingresos Totales (Ventas)</FormLabel>
                                            <FormControl><Input type="number" {...field} /></FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )} />
                                    <FormField control={form.control} name="total_expense" render={({ field }) => (
                                        <FormItem>
                                            <FormLabel>Gastos (Materia Prima)</FormLabel>
                                            <FormControl><Input type="number" {...field} /></FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )} />
                                ...
                                {records.length === 0 ? (
                                    ...
                                    records.map((record) => (
                                        <TableRow key={record.id} className="hover:bg-muted/30 transition-colors group">
                                            ...
                                            <TableCell className="text-destructive font-medium">${Number(record.total_expense).toLocaleString()}</TableCell>
                                            <TableCell className="font-bold">
                                                ${(Number(record.total_income) - Number(record.total_expense)).toLocaleString()}
                                            </TableCell>
                                            <TableCell className="text-right">
                                                <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <Button variant="outline" size="icon" className="h-8 w-8 cursor-pointer" onClick={() => {
                                                        ...
                                                        form.setValue("total_expense", record.total_expense)
                                                        setIsAddOpen(true)
                                                    }}>
